...
| **Drift** (scheduled) | cron (hourly scoped + weekly broad) | read-only API creds | not used | `drift [RESOURCE=…]` → exit 3 → backfill PR (`drift_summary` + `audit` body) → `assert-clean` gates auto-merge |
