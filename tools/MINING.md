...
| ZPA plans show `&amp;`/`&gt;` updates on name/description | the Go SDK HTML-unescapes ZPA and ZCC responses — top-level name/description only, applied TWICE (`unescapeHTML` in zscaler-sdk-go `zscaler/utils.go`, called from `zparequests.go`/`zccrequests.go`; zia has no such call) — so state holds literals while the raw API carries entities. **CAVEAT (field-hit twice):** the unescape is a NO-OP when the read's `v` is a pagination wrapper or slice — resources whose provider Read uses GetAll/list (zpa_app_connector_group, zcc forwarding profiles) keep ESCAPED state, so their config must stay escaped. Check the Read's SDK call shape per resource before assuming the product-wide rule | transform mirrors it (`_unescape_html_fields`), per-resource opt-out via `no_html_unescape`; `make lint` warns on residual entities |
