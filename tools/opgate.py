...
def cmd_validate(slug, tenant, runner=None):
    ...
    status = "pass" if not blocking else "blocked"
    if status == "pass":
        next_step = ("validate passed (static checks only -- the downstream "
                     "plan review can still block) -- proceed to PR (G3): "
                     "commitback checkpoint + DRAFT PR; never terraform apply")
    else:
