...
| ZCC provider fails to authenticate (legacy `ZCC_*` keys ignored) | The zcc provider (0.1.0-beta.1) is OneAPI-ONLY — its config surface is `ZSCALER_*` (Zidentity) with no legacy wiring, even though the SDK and `make fetch` both support legacy ZCC keys. Terraform needs a Zidentity API client entitled for Client Connector; a READ-ONLY role suffices for the whole import phase (imports write state, never the tenant — assert-clean proves it). Until entitlement lands, run ZCC read-only: fetch+transform+drift with legacy keys keeps config current; bootstrap when OneAPI arrives |
