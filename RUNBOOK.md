...
2. **Drift (non-zero run + changed worktree) → branch → PR.** The
   branch/commit/push/PR mechanics live in `pipelines/commitback.sh` —
...
| `cred_env` reports a product's credentials are incomplete, or `missing required env var <ZIA_API_KEY\|ZIA_USERNAME\|ZIA_PASSWORD\|ZIA_CLOUD\|ZPA_CLIENT_ID\|ZPA_CLIENT_SECRET\|ZPA_CUSTOMER_ID\|ZPA_CLOUD>` after setting `ZSCALER_USE_LEGACY_CLIENT` | `cred_env` now preflights per-mode completeness and names EVERY gap per product before fetch runs (it no longer dumps whatever it finds): each legacy product (ZIA, ZPA) is all-or-nothing — a half-set product is a loud, named error; a product with no credentials is tolerated (not required). That tolerance is about resolution, not fetch scope: `make fetch` still attempts every product unless you pass `RESOURCE` (e.g. `RESOURCE=zia` for a ZIA-only legacy tenant), so set `RESOURCE` to keep unconfigured products from failing auth. ZCC is OneAPI-only and has no legacy vars. See `tools/FETCH.md` **Legacy** section |
...
| ZCC provider fails to authenticate (legacy `ZCC_*` keys ignored), or `make fetch` rejects ZCC in legacy mode | ZCC is OneAPI-only end to end: the provider (0.1.0-beta.1) AND `make fetch` both reject legacy ZCC keys — its only config surface is `ZSCALER_*` (Zidentity). In legacy mode a ZCC fetch fails loud; scope it out with `RESOURCE="zia zpa"` until you migrate the tenant to OneAPI. Terraform needs a Zidentity API client entitled for Client Connector; a READ-ONLY role suffices for the whole import phase (imports write state, never the tenant — assert-clean proves it). Until entitlement lands, leave ZCC out of legacy runs and bootstrap when OneAPI arrives |
| `make lint` exits 1 in a fetch+transform pipeline (errors in freshly-pulled data) | The findings are REAL — they describe the tenant's existing data (edge whitespace, pasted separators, NBSP) and make a console-cleanup worklist — but importing that data as-is is correct. Point the pipeline's gate step at `make refresh-gates TENANT=<label>` (advisory lint + strict typecheck; reports everything, suppresses the lint exit, loudly). The strict default remains the PR gate for hand-edited config. LOG SIGNATURE: advisory mode prints an `ADVISORY MODE: N error(s) reported but exit suppressed` line after the summary — if the step still fails WITHOUT that line, the variable never reached lint: pulling the repo updates `tools/` and the EXAMPLE yml, not an adapted pipeline definition. That one-time YAML edit (inline gates → `make refresh-gates`) is also the last one: from then on gate behavior changes ride the repo pull |
| Commit-back/backfill step hangs, or pushes a branch but never opens the PR | The inline block in an adapted pipeline rolled its own mechanics — replace the step body with `bash pipelines/commitback.sh` (env: `SYSTEM_ACCESSTOKEN: $(System.AccessToken)` mapped explicitly, plus `TENANT`/`BRANCH_PREFIX`/`PR_TITLE`; see the example YAMLs). The script cannot hang by construction: no az CLI (extension add hangs on egress, `pr create` waits on stdin, telemetry hangs AFTER success), stdin closed + `GIT_TERMINAL_PROMPT=0` (credential prompts fail loud), curl time-capped with the HTTP status checked. Diagnosis is the last `[commit-back N/5]` checkpoint printed — each names its step. The classic on proxied networks: push succeeds, REST call dies — agent-level git proxy config carries the push, but curl reads ONLY the env; checkpoint 5/5 prints `https proxy: set`/`unset` (never the value), and `unset` on a proxied agent means the step is missing its `HTTPS_PROXY` declaration |
...
| Fresh import of ISOLATE url-filtering rules plans `+ cbi_profile` (the one non-import delta) | Provider-documented API bug: the detail GET omits `cbiProfile` for ISOLATE rules, and the provider's preserve-from-state workaround has nothing to preserve on a fresh import. Config (from the list endpoint) carries the rule's real profile. Two dispositions: (a) shops that can apply a semantic no-op write — APPROVE it (re-asserts the identical profile; converges permanently); (b) ZERO-WRITE shops — converge in state instead (PRE-FLIGHT: re-fetch + re-transform so the committed config is CURRENT — the fill copies it): `terraform -chdir=envs/<label>/zia_url_filtering_rules import 'module.zia_url_filtering_rules.zia_url_filtering_rules.this["<key>"]' <rule id>` (state-only), then `make statefill TENANT=<label> RESOURCE=zia_url_filtering_rules KEY=<key> FIELD=cbi_profile STATE_FILL=1` (copies the committed config value into state — fills absent values only, never overwrites), then `make stage-imports STATE_AWARE=1` (filters the now-managed address) and re-plan: imports-only. Either way the provider REQUIRES `cbi_profile` on ISOLATE config — never drop it |
