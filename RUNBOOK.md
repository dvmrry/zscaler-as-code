...
| `make lint` exits 1 in a fetch+transform pipeline (errors in freshly-pulled data) | The findings are REAL — they describe the tenant's existing data (edge whitespace, pasted separators, NBSP) and make a console-cleanup worklist — but importing that data as-is is correct. Run the pipeline's lint step with `LINT_ADVISORY=1` (reports everything, suppresses the exit, loudly). The strict default remains the PR gate for hand-edited config |
